...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <?php else: ?>
        ...
        function loadMinorCategories(mcId) {
            ...
            if (mcId) {
                fetch('<?= url_to('InventoryController::getMinorCategories') ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'X-Requested-With': 'XMLHttpRequest'
                    },
                    body: 'mc_id=' + encodeURIComponent(mcId)
                })
        ...
        function loadProducts(micId) {
            ...
            if (micId) {
                fetch('<?= url_to('InventoryController::getProducts') ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'X-Requested-With': 'XMLHttpRequest'
                    },
                    body: 'mic_id=' + encodeURIComponent(micId)
                })
                .then(response => response.json())
                .then(data => {
                    if (data.status === 'success') {
