...
<div class="container py-4">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form onsubmit="search('<?= url_to('ManufacturerInventoryController::index') ?>'); return false;">
                <div class="row g-3 align-items-end">
                    <div class="col-md-3">
                        <label for="keyword" class="form-label">關鍵字</label>
                        <input type="text" class="form-control" id="keyword" name="keyword" placeholder="輸入廠商名稱、產品名稱" value="<?= esc($filter['keyword'] ?? '') ?>">
                    </div>
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-primary me-2">
                            <i class="bi bi-search"></i> 搜尋
                        </button>
                        <button type="button" class="btn btn-secondary" onclick="clearSearch('<?= url_to('ManufacturerInventoryController::index') ?>')">
                ...
                <?php else: ?>
                    <?php foreach ($data as $item): ?>
                        <tr>
                            ...
                            <td><?= esc($item['mi_qty']) ?></td>
                            <td><?= esc($item['mi_initial']) ?></td>
                            <td><?= esc($item['creator']) ?></td>
                            <td><?= esc($item['mi_create_at']) ?></td>
    ...
    <?= view('components/pagination', [
        'pager' => $pager,
        'baseUrl' => url_to('ManufacturerInventoryController::index'),
        'params' => $filter
    ]) ?>
</div>
